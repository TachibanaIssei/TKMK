...
- [6. 敵CPU](#6-敵cpu)
  - [6.1. 評価値](#61-評価値)
    - [6.1.1. 評価値を計算する](#611-評価値を計算する)
    - [6.1.2. 剣士の数だけ評価値の計算を行う](#612-剣士の数だけ評価値の計算を行う)
    - [6.1.3. 評価値を比較しターゲットを決定する](#613-評価値を比較しターゲットを決定する)
- [7. 技術紹介(グラフィックス)](#7-技術紹介グラフィックス)
  - [7.1. デプスシャドウ](#71-デプスシャドウ)
    - [7.1.1. シャドウマップを作成する](#711-シャドウマップを作成する)
    - [7.1.2. ライトビュースクリーン空間でのZ値を計算する](#712-ライトビュースクリーン空間でのz値を計算する)
    - [7.1.3. シャドウマップに書き込まれているZ値と比較する](#713-シャドウマップに書き込まれているz値と比較する)
  - [7.2. 川瀬式ブルームフィルター](#72-川瀬式ブルームフィルター)
    - [7.2.1. モデルを描画する](#721-モデルを描画する)
    - [7.2.2. 輝度抽出をする](#722-輝度抽出をする)
    - [7.2.3. 輝度抽出したテクスチャにガウシアンブラーをかけボケ画像を作成](#723-輝度抽出したテクスチャにガウシアンブラーをかけボケ画像を作成)
    - [7.2.4. 3で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成](#724-3で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成)
    - [7.2.5. 4で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成](#725-4で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成)
    - [7.2.6. 5で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成](#726-5で作成したボケ画像にさらにガウシアンブラーをかけボケ画像を作成)
    - [7.2.7. 4枚のボケ画像の合成し、シーンに加算合成する](#727-4枚のボケ画像の合成しシーンに加算合成する)
- [8. U22プログラミング・コンテスト2023に向けた個人での取り組み](#8-u22プログラミングコンテスト2023に向けた個人での取り組み)
- [9. 複数人プレイ](#9-複数人プレイ)
  - [9.1. 画面分割](#91-画面分割)
    - [9.1.1. ビューポートを用意する](#911-ビューポートを用意する)
    - [9.1.2. ビューポートにモデルを描画する](#912-ビューポートにモデルを描画する)
...
- [11. PBR(物理ベースレンダリング)](#11-pbr物理ベースレンダリング)
  - [PBRのワークフロー](#pbrのワークフロー)
  - [拡散反射光](#拡散反射光)
  - [鏡面反射光](#鏡面反射光)
  - [ライティングを計算する](#ライティングを計算する)
- [12. ディファードライティング](#12-ディファードライティング)
...
メタリックと滑らかさは1つのテクスチャにまとめて、メタリックはRチャンネル、滑らかさはαチャンネルを利用します。<br>
...
## 鏡面反射光
今回実装した鏡面反射はCook-Torranceモデルを利用しています。
...
## ライティングを計算する
滑らかさを使用して拡散反射光と鏡面反射光を合成し、ライティングの計算を行います。
