...
メタリックと滑らかさは1つのテクスチャにまとめて、メタリックはRチャンネル、滑らかさはαチャンネルを利用します。
...
次に、カメラの位置(視点)についてです。拡散反射がより強く目に飛び込んでくるのは「サーフェイスから目までのベクトルと、光の入射ベクトルがなす角度が小さいとき」です。つまり、拡散反射は「ひかりの入射ベクトルとサーフェイスの法線、そして目までのベクトルが近いほど強い」といえます。
