		...
		if (oldLv != Lv) {
			...
			if (GetIsGroundFlag() == false) {
				...
				if (IsActorGroundChack() == true) {
					...
					//ジャンプ
					if (pushFlag == false && m_charCon.IsOnGround() && g_pad[playerNumber]->IsTrigger(enButtonA))
					{
						pushFlag = true;
			...
			if (CantMove == false)
			{
				stickL.x = g_pad[playerNumber]->GetLStickXF();
				stickL.y = g_pad[playerNumber]->GetLStickYF();
			}
...
void KnightPlayer::Attack()
{
	...
	{
		if (g_pad[playerNumber]->IsTrigger(enButtonA)&& m_AtkTmingState!= SecondAtk_State)
		{
	...
	{
		if (g_pad[playerNumber]->IsTrigger(enButtonA)&&m_AtkTmingState != LastAtk_State)
		{
	...
	//スキルを発動する処理
	//Bボタンが押されたら
	if (pushFlag == false && SkillEndFlag==false && SkillState == false && g_pad[playerNumber]->IsTrigger(enButtonB)&&m_Status.Hp>0)
	{
		//スキルを使うときのスピードを使う
	...
	//必殺技を発動する処理
	//Xボタンが押されたら
	if (pushFlag == false && Lv >= 4 && g_pad[playerNumber]->IsTrigger(enButtonX))
	{
		//画面を暗くする
...
void KnightPlayer::Avoidance()
{
	...
	if (pushFlag == false && AvoidanceEndFlag == false && AvoidanceFlag == false) {
		if (g_pad[playerNumber]->IsTrigger(enButtonRB1) || g_pad[playerNumber]->IsTrigger(enButtonLB1)) {
			//回避ステート
